...
export default function LoginPage() {
  ...
  async function handleGoogleLogin() {
    try {
      ...
      // OAuth redirects to /auth/callback — no need to handle here
    } catch (error) {
      toastError(getAuthErrorMessage(error))
    }
  }
    ...
    <div className="page-enter min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-text-secondary mt-1">Sign in to continue shopping</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="card space-y-5">
          ...
          <div className="flex items-center justify-between text-sm">
            <label className="flex items-center gap-2 text-text-secondary">              <input
                type="checkbox"
