...
// FIX: Removed `full_name` field entirely.
// admin.service.js createAdmin only destructures { email, password } —
// full_name is never used, never stored, and never returned by getAdmins
// (which selects `id, email, role, created_at` only).
...
// FIX: Delete confirmation now uses admin.email instead of admin.full_name,
// since getAdmins never returns full_name (only id, email, role, created_at).
    ...
    <Modal title="Delete Admin" onClose={onClose}>
      <div className="space-y-4">
        <p className="text-sm text-text-secondary">
          Remove <span className="font-semibold text-text-primary">{admin.email}</span> as an admin?
...
export default function AdminAdminsPage() {
  ...
  const fetchAdmins = useCallback(async () => {
    setLoading(true)
    try {
      // Backend returns an array directly (admin.service.js getAdmins returns data, not { admins: data })
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-border/60 shadow-sm divide-y divide-border">
          {admins.map(admin => {
            // FIX: isSelf comparison by id or email (no full_name available)
            const isSelf = admin.id === user?.id || admin.email === user?.email
            // FIX: derive initials from email since full_name is never returned
              ...
              <div key={admin.id} className="flex items-center gap-4 px-4 py-3 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-text-primary text-sm flex items-center gap-2">
                    {/* FIX: was admin.full_name ?? '—' (always '—'). Show email as primary identifier. */}
                    {admin.email}
                    {isSelf && (
                  ...
                  <p className="text-xs text-text-secondary truncate capitalize">{admin.role}</p>
                </div>
                ...
                <div className="relative group shrink-0">
                  <button
                    ...
                    disabled={isSelf}
                    aria-label={isSelf ? 'Cannot delete own account' : `Delete ${admin.email}`}
                    className={`
